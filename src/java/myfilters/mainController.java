 ...
 * @author Hoang Anh
 */
@WebFilter(filterName = "mainController", urlPatterns = {"/*"})
public class mainController implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        try {
            ...
            url=path.substring(index+1);
            if(url.isEmpty())
                url="index.html";
            else if (!url.contains("html")& !url.contains("jsp")  & !url.contains("css") & !url.contains(".jpg")& !url.contains(".png"))
                url=url+"Servlet";
             else if(url.contains("&")){
                int index2=path.lastIndexOf("&");
                url=path.substring(index+1, index2);
                url=url+"Servlet";
                        }
            if(url!=null)
